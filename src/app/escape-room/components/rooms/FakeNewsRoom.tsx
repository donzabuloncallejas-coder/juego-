...
const pieces: Piece[] = [
	{ id: 'fuente', clue: 'Portal oficial o medio con politica editorial visible', emoji: '🔎' },
	{ id: 'fecha', clue: 'Fecha clara y reciente en la publicacion', emoji: '📅' },
	{ id: 'autor', clue: 'Nombre real y perfil rastreable del redactor', emoji: '🧑‍💻' },
	{ id: 'evidencia', clue: 'Datos con enlace a informe, estudio o documento', emoji: '🧾' }
];

const slots: Slot[] = [
	{ id: 'fuente', title: 'Origen', hint: '¿De donde sale la informacion?' },
	{ id: 'fecha', title: 'Tiempo', hint: '¿Cuando fue publicada?' },
	{ id: 'autor', title: 'Responsable', hint: '¿Quien firma el contenido?' },
	{ id: 'evidencia', title: 'Soporte', hint: '¿Con que lo comprueban?' }
];
...
export default function FakeNewsRoom({ onComplete }: FakeNewsRoomProps) {
	...
	useEffect(() => {
		if (!completed && placed.length === slots.length && challengePassed) {
			setCompleted(true);
			onComplete();
		}
	...
	const canVerify = placed.length === slots.length && challengeSelection !== null;
		...
		>
			<section style={{ display: 'grid', gap: '0.85rem' }}>
				...
				<div className="glass-card" style={{ padding: '0.8rem' }}>
					<h3 style={{ marginTop: 0 }}>Pistas de verificacion</h3>
